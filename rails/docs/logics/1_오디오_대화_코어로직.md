...
**Echo 카드 구조:**
- 왼쪽 영역: 오디오 플레이 버튼
- 오른쪽 영역: Echo 상세 화면으로 이동
  - 남이 올린 Echo: 1:1 대화 화면 (`/echos/:id`)
  - 내가 올린 Echo: 복수 대화방 목록 (`/echos/:id`)
...
**관련 파일:**
- `app/views/feed/index.html.erb` - 피드 메인 화면
- `app/views/feed/_echo.html.erb` - Echo 카드 (조건부 링크 렌더링)
- `app/controllers/echos_controller.rb` (`show` 액션) - Echo 상세 로직
- `app/views/echos/show.html.erb` - Echo 상세 화면 (Case 1: ChatRoom 목록, Case 2: 1:1 대화)
- `config/routes.rb` - echos 라우트
- `ios/voice_talk_ios/path-configuration.json`, `android/.../path-configuration.json` - /echos 경로 설정
...
**관련 파일:**
- `app/controllers/messages_controller.rb` (`create` 액션) - ChatRoom 생성 및 메시지 처리
- `app/controllers/messages_controller.rb` (`create_chat_room_with_echo_copy` 메소드) - 첫 답장 시 ChatRoom 생성
- `app/views/messages/_chat_subscription.html.erb` - Action Cable 구독 partial
- `app/models/chat_room.rb`
- `app/models/message.rb`
- `config/routes.rb` - echos/:id/messages 라우트
...
**관련 파일:**
- `app/controllers/messages_controller.rb` (`create` 액션의 턴제 검증 로직)
- `app/models/message.rb` (`broadcast_to_chat_room` 메소드) - Action Cable broadcast
- `app/views/shared/footer_waiting.html.erb` - 대기 중 UI
- `app/views/shared/error_message.html.erb` - 에러 메시지 partial
...
- 각 ChatRoom 클릭 시 `/chat_rooms/:id`로 이동하여 대화 내용 표시

**`/chat_rooms/:id` 화면 (ChatRoomsController#show):**
- Echo 오디오 플레이어 (상단 고정)
- Messages 목록
- 턴제 답장 UI (footer)
- Action Cable을 통한 실시간 메시지 수신
- `/echos/:id`의 1:1 대화 화면(Case 2)와 동일한 구조

**라우팅:**
- `/echos/:id` → EchosController#show (ChatRoom 목록 or 1:1 대화)
- `/chat_rooms/:id` → ChatRoomsController#show (특정 ChatRoom 대화)
- POST `/chat_rooms/:id/messages` → MessagesController#create

**관련 파일:**
- `app/controllers/chat_rooms_controller.rb` - ChatRoom 상세 화면
- `app/views/chat_rooms/show.html.erb` - ChatRoom 대화 화면
- `app/views/echos/show.html.erb` (Case 1) - ChatRoom 목록 표시
- `app/controllers/messages_controller.rb` (`create` 액션) - echo_id/chat_room_id 양쪽 지원
- `app/models/chat_room.rb`
- `config/routes.rb` - chat_rooms 라우트
- `ios/voice_talk_ios/path-configuration.json`, `android/.../path-configuration.json` - /chat_rooms 경로 설정
...
**내가 올린 Echo의 대화방:**
- `/messages` 목록에서는 제외 (남이 올린 Echo의 대화방만 표시)
- 내가 올린 Echo의 대화방은 `/echos/:id`에서 접근

**관련 파일:**
- `app/controllers/messages_controller.rb` (`index` 액션)
- `app/views/messages/index.html.erb` - 대화방 목록 화면
- `config/routes.rb` - messages 라우트
...
**관련 파일:**
- `app/models/chat_room.rb` (`user_left?` 메소드)
...
- `/chat_rooms/:id`: Message 생성 (`submit_url: chat_room_messages_path(@chat_room)`)
- `/messages`: 숨김 or 비활성
- `/settings`: 숨김 or 비활성
...
**관련 파일:**
- `app/views/shared/_footer.html.erb` - 녹음 UI
- `app/views/shared/_footer_waiting.html.erb` - 대기 중 UI
- `app/controllers/echos_controller.rb` (`create` 액션) - Echo 생성
- `app/controllers/messages_controller.rb` (`create` 액션) - Message 생성
- `app/javascript/controllers/bridge/audio_recording_controller.js` - 네이티브 녹음 Bridge
- `ios/voice_talk_ios/Bridge/Components/AudioRecordingComponent.swift` - iOS 녹음 구현
...
### 모델 (app/models/)
- `echo.rb` - Echo 모델
- `chat_room.rb` - ChatRoom 모델
- `message.rb` - Message 모델 (Action Cable broadcast 포함)
- `recording.rb` - Recording 모델 (Polymorphic)
- `user.rb` - User 모델

### 컨트롤러 (app/controllers/)
- `feed_controller.rb` - 피드 목록
- `echos_controller.rb` - Echo 생성 및 상세 (ChatRoom 목록 or 1:1 대화)
- `chat_rooms_controller.rb` - ChatRoom 상세 대화 화면
- `messages_controller.rb` - Message 생성 및 대화방 목록
- `settings_controller.rb` - 설정 화면

### 뷰 (app/views/)
- `feed/index.html.erb` - 피드 메인 화면
- `feed/_echo.html.erb` - Echo 카드 partial
- `echos/show.html.erb` - Echo 상세 (Case 1: ChatRoom 목록, Case 2: 1:1 대화)
- `chat_rooms/show.html.erb` - ChatRoom 대화 화면
- `messages/index.html.erb` - 대화방 목록
- `messages/_message.html.erb` - 메시지 partial
- `messages/_chat_subscription.html.erb` - Action Cable 구독 partial
- `shared/_footer.html.erb` - 녹음 UI
- `shared/_footer_waiting.html.erb` - 대기 중 UI

### JavaScript (app/javascript/controllers/)
- `audio_player_controller.js` - 오디오 재생
- `pull_to_refresh_controller.js` - Pull to refresh
- `infinite_scroll_controller.js` - Infinite scroll
- `bridge/audio_recording_controller.js` - 네이티브 녹음 Bridge
- `bridge/navigation_bar_controller.js` - Navigation bar 제어 Bridge

### 네이티브 (iOS)
- `voice_talk_ios/Bridge/Components/AudioRecordingComponent.swift` - 녹음 구현
- `voice_talk_ios/Bridge/Components/NavigationBarComponent.swift` - Navigation bar 제어
- `voice_talk_ios/path-configuration.json` - 경로 설정

### 네이티브 (Android)
- `android/app/src/main/assets/json/path-configuration.json` - 경로 설정
